class HomeController < ApplicationController
  ...
  def schools
    ...
    @share = "Amb dades dels últims 15 dies, #{@schools.select{|s| s["mean"].to_f < 40.0}.count} de les #{@schools.count} escoles#{@level_name ? (@level_name[0] =~ /^[aeiou]/i ? " d'" : " de ") + "#{@level_name[0]} " : ""} #{(@district ? (@district == 'Eixample' ? "de l'#{@district}" : (@district == 'Horta-Guinardó' ? "d'#{@district}" : "de #{@district}")) : "de Barcelona")} (#{(100 * @schools.select{|s| s["mean"].to_f < 40.0}.count/@schools.count.to_f).round}%) es troben per sobre els 40 µg/m3 de NO2 durant l'horari escolar. Consulta la llista de les escoles més contaminiades a: contaminacio.cat/escoles"
